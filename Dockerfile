...
ENV GEM_PATH /app/heroku/ruby/bundle/ruby/2.1.0
ENV GEM_HOME /app/heroku/ruby/bundle/ruby/2.1.0
RUN mkdir -p /app/heroku/ruby/bundle/ruby/2.1.0

# Install Ruby
RUN mkdir -p /app/heroku/ruby/ruby-2.1.5
RUN curl -s --retry 3 -L https://heroku-buildpack-ruby.s3.amazonaws.com/cedar-14/ruby-2.1.5.tgz | tar xz -C /app/heroku/ruby/ruby-2.1.5
ENV PATH /app/heroku/ruby/ruby-2.1.5/bin:$PATH
...
ENV PATH /app/user/bin:/app/heroku/ruby/bundle/ruby/2.1.0/bin:$PATH
